...
app.post('/JoinRequest', (req, res) => {
    ...
    var PHYPayload = loraPacket.fromFields(
        ...
        undefined, undefined
    ).getPHYPayload().toString('base64')
    var resp = {PHYPayload:PHYPayload}
    res.setHeader('Content-Type', 'application/json');
    res.send(JSON.stringify(resp));
});
...
app.post('/UnconfirmedDataUp', (req, res) => {
    var data = req.body;
    console.log(data)
    var packet = loraPacket.fromFields(
        {
    ...
    //var encryptedPacket = packet.encrypt(data.AppSKey, data.NwkSKey)
    var PHYPayload = packet.getPHYPayload()
    var resp = {PHYPayload:PHYPayload.toString('base64'), verifyMIC: loraPacket.verifyMIC(packet, Buffer.from(data.NwkSKey, 'hex'), undefined, undefined)}
    res.setHeader('Content-Type', 'application/json');
    res.send(JSON.stringify(resp));
});
...
app.post('/ConfirmedDataUp', (req, res) => {
    ...
    var PHYPayload = loraPacket.fromFields(
        ...
        data.AppSKey, data.NwkSKey,
        undefined, undefined, undefined
    ).getPHYPayload();
    var resp = {PHYPayload:PHYPayload.toString('base64')}
    res.setHeader('Content-Type', 'application/json');
    res.send(JSON.stringify(resp));
});
